...
class resnetv1(network):
  def __init__(self, num_layers=50):
    ...
    #feature stride is recalculated as rpn is 8..
    #self._feat_stride = [8, ]
  ...
  def _crop_pool_layer(self, bottom, rois, name):
    with tf.variable_scope(name) as scope:
      ...
      crops = tf.image.crop_and_resize(bottom, bboxes, tf.to_int32(batch_ids), [POOLING_SIZE, POOLING_SIZE],
                                         name="crops")
    return crops
  ...
  def _build_base(self):
    with tf.variable_scope(self._scope, self._scope):
      ...
      #net = resnet_utils.conv2d_same(self._image, 64, 3, stride=2, scope='conv1')
  ...
  def _decide_blocks(self):
    ...
    if self._num_layers == 50:
      ...
      self._blocks = [resnet_v1_block('block1', base_depth=64, num_units=3, stride=2),
                      ...
                      #resnet_v1_block('block4', base_depth=512, num_units=6, stride=1)]
                      resnet_v1_block('block4', base_depth=512, num_units=3, stride=1)]
      #]
